...
*   ### 9 - update sold values for each product
        # prod 4 intentionally fails, should correctly be captured
    ### 10 - check if values were updated
    ### 11 - prints ticket -> emulate success
    ### 12 - Delete the ticket
"""


def test_e2e_fail_create_update_sold(app):
    ### 1 - opens program (login screen)
    ...
    open_view_ticket.product_widgets[1]['sold_edit'].setText(prod_1_sold_qty) # the widget index is the product id
    open_view_ticket.product_widgets[1]['update_btn'].click()

    open_view_ticket.product_widgets[2]['sold_edit'].setText(prod_2_sold_qty)
    open_view_ticket.product_widgets[2]['update_btn'].click()

    open_view_ticket.product_widgets[3]['sold_edit'].setText(prod_3_sold_qty)
    open_view_ticket.product_widgets[3]['update_btn'].click()

    open_view_ticket.product_widgets[4]['sold_edit'].setText(prod_4_sold_qty)
    with patch("ui.core.view_ticket.QMessageBox.warning") as fail_warning:
        open_view_ticket.product_widgets[4]['update_btn'].click()
        assert fail_warning.called, "Expected failure: sold qty exceeds signed qty"

    ### 10 - check if values were updated
    result = get_item("Consignments", 'consignment', ticket_number)
    assert result['products'][0]['sold'] == int(prod_1_sold_qty), "Expected sold to be updated for product 1"
    assert result['products'][1]['sold'] == int(prod_2_sold_qty), "Expected sold to be updated for product 2"
    assert result['products'][2]['sold'] == int(prod_3_sold_qty), "Expected sold to be updated for product 3"
    assert result['products'][3]['sold'] == 0, "Expected sold to not be updated for product 4"
