...
class PostTab(BaseTab):
    ...
    def update_revenue_fields(self) -> int:
        ...
        """
        try:
            # 1) Compute subtotal from product lines
            subtotal = 0.0
            ...
            # Subtotal can be zero; still fill rows with 0.00
            # 2) For each revenue row, use SXC-22 method
            # Your widget stores rows in self.revenue_generation.revenue_records
            ...
            # 3) Iterate rows and populate fields
            for rec in records:
                pct_label = rec['percentage'].text()  # e.g., "25%"
                result = calc(subtotal, 1, pct_label)  # price*qty == subtotal -> (subtotal * 1)
                ...
                # Overwrite "Autogenerated" with formatted values (currency-like)
